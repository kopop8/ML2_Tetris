...
import pickle
def train_agent(current_generation,num_generations,sol_per_pop ,  pop_size, new_population, num_parents_mating, num_mutations,mutate_percentage, isMultilayer,data, max_lines_cleared=False):
    print("Beginning training")
    best_outputs = []
    num_generations = current_generation+ num_generations
    ...
    # print(new_population)
        ...
        data.append([generation+1,np.max(fitness),np.average(fitness),new_population])   
...
def GET_best_weights(isMultilayer):
    ...
    total_data = pickle.load(pkl_file)
    data = total_data[-1]
    pkl_file.close()
    ...
    return data, current_generation, total_data

if useSave:
    new_population, current_generation, total_data = GET_best_weights(isMultilayer)
else:
    new_population = np.random.uniform(low=-4.0, high=4.0, size=pop_size)
print(current_generation)
print(len(total_data))
train_agent(current_generation,num_generations, sol_per_pop, pop_size, new_population, num_parents_mating, num_mutations, mutate_percentage, isMultilayer,total_data, max_lines_cleared)
